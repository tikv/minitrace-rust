...
mod tests {
    ...
    fn span_stack_basic() {
        ...
        let span_line1 = span_stack
            .register_span_line(Some(new_collect_token([token1])))
            .unwrap();
        {
            {
            ...
            let span_line2 = span_stack
                .register_span_line(Some(new_collect_token([token2])))
                .unwrap();
            {
        ...
        let span_line1 = span_stack.register_span_line(None).unwrap();
        {
            let span_line2 = span_stack.register_span_line(None).unwrap();
            {
                let span_line3 = span_stack
                    .register_span_line(Some(new_collect_token([CollectTokenItem {
                        parent_id_of_roots: SpanId::new(9527),
                        collect_id: 42,
                    }])))
                    .unwrap();
                ...
                {
                    let span_line5 = span_stack
                        .register_span_line(Some(new_collect_token(None)))
                        .unwrap();
                    {
                        assert!(span_stack
                            .register_span_line(Some(new_collect_token([CollectTokenItem {
    ...
    fn current_collect_token() {
        ...
        );
        {
            ...
            assert!(span_stack.current_collect_token().is_none());
            {
                ...
                let span_line3 = span_stack
                    .register_span_line(Some(new_collect_token([token3])))
                    .unwrap();
                assert_eq!(
                    span_stack.current_collect_token().unwrap().as_slice(),
                    &[token3]
            ...
            let span_line4 = span_stack
                .register_span_line(Some(new_collect_token([token4])))
                .unwrap();
            assert_eq!(
                span_stack.current_collect_token().unwrap().as_slice(),
                &[token4]
